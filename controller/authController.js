...
exports.signUp = async (req, res, next) => {
    ...
    try {
        ...
        if (existingUser) {
            ...
            return res.status(409).json({ status: false, message: 'User Already Exists With This Phone Number', business_owner: {} });
        }
        ...
        return res.status(201).json({ status: true, message: 'Registration Success', business_owner: new_user });
    } catch (error) {
...
exports.signIn = async (req, res, next) => {
    ...
    try {
        ...
        if (!compare_password) {
            await transactionScope.rollback()
            return res.status(401).json({ status: false, message: 'Password does not match', business_owner: {} })
        }

        const token = jwt.sign({ business_owner_data: user.id, }, "tokenSecretKey")
...
exports.resetPassword = async (req, res, next) => {
    ...
    try {
        ...
        return res.status(201).json({ status: true, message: 'Password Update Success', business_owner: user })
...
exports.updateBusinessProfile = async (req, res, next) => {
    ...
    try {
        ...
        await transactionScope.commit();
        return res.status(201).json({ status: true, message: 'Business Profile updated successfully', business_owner: updatedUser });
    } catch (error) {
        if (transactionScope) await transactionScope.rollback();
        console.log(error)
...
// make business owner premium 

exports.makePremium = async (req, res, next) => {
    const business_owner_id = req.business_owner_id;
    const { is_premium, subscription_expiry_date } = req.body;
    const transactionScope = await sequelize.transaction();
    ...
    try {
        ...
        await transactionScope.commit();
        return res.status(201).json({ status: true, message: 'This User is Premium Now', business_owner: updatedUser });
    } catch (error) {
        if (transactionScope) await transactionScope.rollback();
        console.log(error)
        return res.status(500).json({ status: false, message: 'Server Error', user: {} });
